...

{
	...
	chop($server_url); #remove last character in our case '/'
	my $test_config_file_content = qq(
	{
		"information_service_url":"$server_url",
		"experiment_manager_user":"dkrol-laptop",
		"experiment_manager_pass":"somepassword",
		"experiment_id":"520f4dcdfc3ff9483c000002"
	} );
	
	diag($test_config_file_content);
	use File::Temp qw/ tempfile tempdir /;
	my $test_tempdir = File::Temp->newdir();
	#diag(`ls -la $test_tempdir`);
	my $fh = File::Temp->new(DIR=>$test_tempdir,SUFFIX=>".config.json");
 	print $fh $test_config_file_content;
 	close($fh); #file empty without this line
 	
 	my $runner = Scalarm::Runner->new(config_file_path => $fh->filename);
 	diag($runner->setup_experiment());
 	#diag($runner->error_threshold);


	# Plain old ASCII
	$t->get_ok('/' => form => {foo => 'yatta'})->status_is(200)->content_is('foo: yatta');
	diag();
...
SKIP: {
	...
	#PREPARE
	can_ok('Scalarm::Runner','setup_experiment');
